...
export function LanguageSelector({ size }: { size?: "default" | "sm" }) {
	...
	function extractParams<T extends string = typeof pathname>(dynamicPath: T, actualPath: string): { [K in ExtractParams<T>]: ParamsFor<T> } {
		...
		const params = {} as { [K in ExtractParams<T>]: ParamsFor<T> };

		dynamicSegments.forEach((segment, i) => {
			if (segment.startsWith("[") && segment.endsWith("]")) {
				const isCatchAll = segment.startsWith("[...") || segment.startsWith("[[...");
				const key = segment.slice(isCatchAll ? 4 : 1, -1) as ExtractParams<T>;

				if (isCatchAll) {
					params[key] = actualSegments.slice(i) as ParamsFor<T>;
				} else {
					params[key] = (actualSegments[i] ?? "") as ParamsFor<T>;
				}
			}
		});
...
type testParams1 = ParamsFor<"/posts/[slug]">; // { slug: string }
//    ^?
