					...
					>
						{navigation.groups?.map((group, key) => {
							...
							const labelContent =
								...
								: showHeader ? <p className="min-w-full font-semibold">{typeof group.header === "function" ? group.header(new Date().getFullYear()) : group.header}</p>
								: null;

							const footerContent = group.footer ? <p className="min-w-full font-semibold">{typeof group.footer === "function" ? group.footer(t) : group.footer}</p> : null;
								...
								>
									...
									<ul
										className={cn("space-y-1", {
											"flex w-full flex-wrap justify-center space-x-1 space-y-0 text-base sm:space-x-6": isGroupHorizontal,
