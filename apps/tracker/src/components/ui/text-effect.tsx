...
const AnimationComponent: React.FC<{
	...
	if (typeof segment !== "string") {
		// render non-text children directly wrapped in motion
		return (
			<motion.span variants={variants} className={cn("inline-block", segmentWrapperClassName)}>
				{segment}
			</motion.span>
		);
	}
...
const splitText = (child: ReactNode, per: PerType): ReactNode[] => {
	if (typeof child !== "string") return [child]; // non-text children stay as-is
	if (per === "line") return child.split("\n");
	return child.split(/(\s+)/);
};

const hasTransition = (variant?: Variant): variant is TargetAndTransition & { transition?: Transition } => typeof variant === "object" && variant !== null && "transition" in variant;
...
export function TextEffect({
	...
	// Normalize children into flat array of text/JSX segments
	const segments = React.Children.toArray(children).flatMap((child) => splitText(child, per));
			...
			{trigger && (
				...
				>
					...
					{segments.map((segment, index) => (
						<AnimationComponent
							key={`${per}-${index}-${typeof segment === "string" ? segment : "jsx"}`}
							segment={segment}
