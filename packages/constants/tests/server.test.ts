...
import { z } from "zod";
import { ConstantsError } from "../src/exceptions/constants-error.ts";
...
const createDefaultInstance = async (overrides = {}) => {
	const { createDefaults } = await import("../src/old/index.js");
...
describe("createDefaults (server-side)", () => {
	describe("client-side usage", () => {
		...
		it("should call onError if called on client-side (window exists)", async () => {
			...
			expect(onErrorMock.mock.calls[0]?.[0].message).toContain("getDefaults cannot be called in the client-side code");
		});
	});
	describe("locale-based defaults", () => {
		...
		] as const)("should return correct defaults for locale '%s'", async (locale, expectedTitle, expectedSubtitle) => {
	...
	describe("error handling", () => {
		it("calls onError callback when defaults don't match schema", async () => {
			...
			).rejects.toThrow(ConstantsError);

			expect(onError).toHaveBeenCalled();
			const error = onError.mock.calls[0]?.[0];
			expect(error).toBeInstanceOf(ConstantsError);
