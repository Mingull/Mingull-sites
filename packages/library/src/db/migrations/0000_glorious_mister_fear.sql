...
CREATE TABLE `preferences` (
	...
	`name` varchar(255) NOT NULL,
	`description` text,
	`type` enum('string','boolean','number','list') NOT NULL,
	`site` varchar(36),
...
CREATE TABLE `user_preferences` (
	...
	`value` text NOT NULL,
	CONSTRAINT `user_preferences_user_id_preference_id_pk` PRIMARY KEY(`user_id`,`preference_id`)
);
--> statement-breakpoint
CREATE TABLE `users` (
	`id` varchar(36) NOT NULL,
...
ALTER TABLE `pastes` ADD CONSTRAINT `pastes_collection_id_collections_id_fk` FOREIGN KEY (`collection_id`) REFERENCES `collections`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `preferences` ADD CONSTRAINT `preferences_site_sites_id_fk` FOREIGN KEY (`site`) REFERENCES `sites`(`id`) ON DELETE cascade ON UPDATE cascade;--> statement-breakpoint
...
ALTER TABLE `user_preferences` ADD CONSTRAINT `user_preferences_preference_id_preferences_id_fk` FOREIGN KEY (`preference_id`) REFERENCES `preferences`(`id`) ON DELETE cascade ON UPDATE no action;