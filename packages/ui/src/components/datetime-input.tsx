...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>((options: DateTimeInputProps, ref) => {
	...
	const onClick = useEventCallback(
		(event: React.MouseEvent<HTMLInputElement>) => {
			...
			if (inputRef.current && selectionStart !== undefined && selectionStart !== null) {
				...
				let segment = validSegments.find((s) => s.index <= selectionStart && s.index + s.symbols.length >= selectionStart);
	...
	const onSegmentChange = useEventCallback(
		(direction: "left" | "right") => {
			...
			const validSegments = segments.filter((s) => s.type !== "space");
			const segment = direction === "left" ? [...validSegments].reverse().find((s) => s.index < curSegment.index) : validSegments.find((s) => s.index > curSegment.index);
			if (segment) {
				setCurrentSegment(segment);
				setSelection(inputRef, segment);
			}
	...
	return (
		<div
			ref={ref}
			className={cn(
				"border-input bg-background ring-offset-background file:text-foreground placeholder:text-muted-foreground flex h-10 items-center justify-start rounded-md border text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:cursor-not-allowed disabled:opacity-50",
				isFocused ? "ring-ring outline-none ring-2 ring-offset-2" : "",
		...
		>
			...
			<input
				...
				className="min-w-0 flex-grow bg-transparent py-1 pe-2 font-mono text-sm focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
			...
			<div className="me-3">
				...
				:	<TooltipProvider>
						<Tooltip>
							...
							<TooltipContent>
								<p>Please enter a valid value. The input cannot be empty and must be within the range of years 1900 to 2100.</p>
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
				}
